...
$fetch_output = '';
$style_typez = 'list1';
$map_listings = $posted_date = $is_featured = $long = $lat = $media = $ratings = '';
$style_typez = dwt_listing_text('dwt_listing_search_layout_style');
//Masonry layout
$map_listings = '<script>var listing_markers = [';
$fetch_output .= '<div class="masonry_container"><div class="masonery_wrap">';
while ($results->have_posts()) {
    $results->the_post();
    $listing_id = get_the_ID();
    $listingz = new dwt_listing_listings();
    $function = "dwt_listing_listing_styles_$style_typez";
    $fetch_output .= $listingz->$function($listing_id, 'no');
    //for map
